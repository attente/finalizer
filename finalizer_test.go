...
import (
	...
	"github.com/attente/finalizer"
	"github.com/stretchr/testify/assert"
)

const (
	TestDuration          = 9 * time.Minute
...
func TestFinalizer(t *testing.T) {
	...
	mempool := finalizer.NewMemoryMempool()

	finalizer, err := finalizer.NewFinalizer(finalizer.FinalizerOptions{
		Wallet:       wallet,
		Chain:        chain,
		Mempool:      mempool,
		Logger:       logger,
